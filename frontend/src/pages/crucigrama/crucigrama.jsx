        ...
        <div className="mini-codycross">
            ...
            <div className="palabra-container">
                {nivel.palabras.map((palabra, i) => {
                    ...
                    return (
                        <div key={i} className={`palabra-row ${palabraSeleccionada === i ? "seleccionada" : ""}`} onClick={() => setPalabraSeleccionada(i)}>
                            {Array.from({ length: palabra.length }).map((_, j) => {
                                const esLetraOculta = j === 2;
                                const letra = arr[j] || "";
                                return (
                                    <span key={j} className={`letra ${completada ? "correcta" : ""} ${esLetraOculta && letra ? "oculta" : ""}`}>
                                        {letra}
                                    </span>
                                );
                            })}
                            <p className="pista">{palabraSeleccionada === i ? "Pista: " + nivel.pistas[i] : ""}</p>
                        </div>
                    );
                })}
            </div>
            ...
            {finalizado && (
                <div className="finalizado">
                    ...
                    <input type="text" value={nombreJugador} onChange={(e) => setNombreJugador(e.target.value)} placeholder="Tu nombre" />
